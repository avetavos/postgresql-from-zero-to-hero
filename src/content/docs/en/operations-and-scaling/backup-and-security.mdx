...
| Option | Benefit | Cost |
|---------|---------|------|
| Automated backups with regular restore tests | Actually recoverable when a real disaster happens | Costs storage and requires operational discipline |
| Relying on replication alone for durability | Fast failover, high availability | A replica is not a backup — it copies mistakes and deletions too |

## Common mistakes

- **Setting up backups once and never testing a restore** — nobody knows a backup is broken until they actually need it. Schedule regular restore drills.
- **Treating a replica as a backup** — a replica tracks the primary in near real time; it is not a separate point-in-time-recoverable copy. Always keep real backups alongside it.
- **Giving an application role broader privileges than it needs (such as using a superuser role for the app)** — follow least privilege, so each role only gets the access it actually needs.

> 💡 Real-world example
>
> **Large-scale social platforms** — treat regular backup and recovery drills as part of standard infrastructure practice once they operate at a scale where an untested backup is not an acceptable risk.
>
> **Common practice on production PostgreSQL teams** — regular, automated restore testing is treated as a standard practice, not just taking the backup itself.
