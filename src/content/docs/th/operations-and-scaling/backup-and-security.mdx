...
description: ปกป้องข้อมูลของคุณด้วย backup แบบ logical และ physical และ point-in-time recovery และ lock มันด้วย role, GRANT และ REVOKE, row-level security, TLS, และ pg_hba.conf
sidebar:
  order: 5
---
...
<Mermaid code={securityDiagram} title="connection ผ่านประตูเครือข่าย, auth, privilege, และ row" />
...
- Backup จะเป็นของจริงก็ต่อเมื่อคุณ restore มันสำเร็จแล้วเท่านั้น จัด table ทดสอบ restore เป็นประจำลงในฐานข้อมูลแบบใช้แล้วทิ้ง และยืนยันว่าข้อมูลยังครบถ้วน
...
|---------|---------|------|
| Automated backup ที่ทดสอบ restore เป็นประจำ | กู้คืนได้จริงเมื่อเกิดภัยพิบัติจริง | เสียพื้นที่ storage และต้องมีวินัยด้าน operations |
| พึ่งพา replication อย่างเดียวเพื่อความทนทานของข้อมูล | failover เร็ว พร้อมใช้งานสูง | replica ไม่ใช่ backup มันคัดลอกทั้งความผิดพลาดและการลบข้อมูลตามไปด้วย |
...
export const quizBackupSecurity = [
  {
    ...
    options: [
      ...
      'การ export แต่ละ table ไปยัง spreadsheet',
    ],
    answer: 1,
  },
  {
    ...
    options: [
      ...
      'row แต่ละ row ใดที่ role อาจเห็นหรือแก้ไขได้',
      'query รันเร็วแค่ไหน',
