...
description: เปลี่ยนข้อความให้เป็น tsvector จับคู่กับ tsquery ด้วย operator @@ จัดอันดับผลลัพธ์ด้วย ts_rank และทำ index การค้นหาด้วย GIN index หรือ column tsvector แบบ generated
sidebar:
  order: 3
---
...
query แบบ `LIKE '%search%'` หา substring ที่ตรงเป๊ะได้ แต่มันไม่เข้าใจภาษา มันบอกไม่ได้ว่า "running" กับ "ran" มีรากศัพท์ร่วมกัน มันไม่ละคำที่ไม่มีความหมายอย่าง "the" และมันจัดอันดับว่า match อันหนึ่งดีกว่าอีกอันไม่ได้ full-text search ของ PostgreSQL แก้ทั้งสามข้อ คือมันย่อข้อความให้เป็นคำค้นที่ถูกทำให้เป็นมาตรฐาน (normalized) จับคู่ query กับคำเหล่านั้น และให้คะแนนว่าแต่ละ row เข้ากันได้ดีแค่ไหน

ในบทนี้เราค้นหา table ของ articles ตามเนื้อความ (body text) ของมัน
...
query ข้างต้นคำนวณ `to_tsvector(body)` ใหม่สำหรับทุก row ในทุกการค้นหา ซึ่งจะช้าลงเมื่อ table โตขึ้น มีสองแพตเทิร์นที่แก้ปัญหานี้ และมันทำงานร่วมกันได้ดี
...
**ใน pgAdmin:** column `search` แบบ generated จะปรากฏใน table เหมือน column อื่น ๆ แต่มันเป็นแบบอ่านอย่างเดียว — คุณแก้ไขค่าของมันโดยตรงไม่ได้เพราะ Postgres ดึงค่ามาจาก `title` และ `body`
...
- คำนวณ `tsvector` ล่วงหน้าใน generated column สำหรับ table ใด ๆ ที่คุณค้นหาซ้ำ ๆ มันย้ายต้นทุนไปอยู่ตอนเขียนและปล่อยให้ GIN index ทำงานของมันได้
...
export const quizFts = [
  ...
  },
  {
    ...
    options: [
      ...
      'ใช้ LIKE แทน',
      'เพิ่ม B-tree index บน column body',
    ],
    answer: 1,
  },
];
