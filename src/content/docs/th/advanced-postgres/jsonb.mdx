...
บางครั้ง row หนึ่งต้องเก็บข้อมูลที่รูปร่างไม่ได้ถูกกำหนดไว้ล่วงหน้า เช่น สินค้าที่มีคุณสมบัติต่างกันไปตามหมวดหมู่ payload ของ event จากบุคคลที่สาม หรือการตั้งค่าของผู้ใช้ที่งอกเพิ่มขึ้นตามเวลา PostgreSQL ให้ column เดียวเก็บเอกสาร JSON ทั้งก้อนด้วยชนิดข้อมูล `jsonb` แล้ว query เข้าไปข้างในได้ราวกับว่า key ของมันคือ column
...
การ insert row ก็เป็น `INSERT` ธรรมดา เอกสารเป็นเพียง JSON string ที่ PostgreSQL parse ให้เป็น `jsonb` จากฝั่ง driver คุณส่ง JSON เป็นพารามิเตอร์แทนการต่อ string ด้วยมือ
...
หากต้องการเปลี่ยนฟิลด์เดียวโดยไม่ต้องเขียนเอกสารทั้งก้อนใหม่ ให้ใช้ `jsonb_set` มันรับ column path ในรูปแบบ text array และค่าใหม่ในรูปแบบ `jsonb`
...
หากไม่มี index คำสั่ง `@>` จะ scan ทุก row และทดสอบแต่ละเอกสาร **GIN** index เปลี่ยนเรื่องนั้น มันทำ index ให้กับ key และ value ภายใน `jsonb` ดังนั้นการค้นหาแบบ containment และ key-existence จึงเร็วขึ้น
...
column `jsonb` ทำให้ table เดียวผสม column ที่เคร่งครัดและมีชนิดข้อมูลชัดเจน เข้ากับฟิลด์เอกสารที่ยืดหยุ่นได้ ความเป็นลูกผสมนั้นมักเป็นความยืดหยุ่นของ schema ทั้งหมดที่แอปพลิเคชันต้องการ — ไม่ต้องมี document database แยกต่างหาก เพียงแค่เก็บส่วนที่คงที่จริง ๆ ไว้ใน column จริงที่ Postgres บังคับใช้ได้
</Callout>
...
- **ยัดทุกอย่างลงใน JSONB blob ก้อนเดียว** — ถ้าข้อมูลมีโครงสร้างจริง ๆ และต้องอ้างอิงไปยัง table อื่น ให้ใช้ column เชิงสัมพันธ์กับ foreign key แทน เพราะ Postgres validate ความสัมพันธ์นั้นให้ไม่ได้ถ้ามันซ่อนอยู่ใน JSONB
...
> 💡 Real World Example
>
> **Notion** — เก็บเนื้อหา block และ page ที่มีโครงสร้างยืดหยุ่นสูงไว้ใน `jsonb` เพราะรูปแบบ block เปลี่ยนบ่อยและหลากหลายเกินกว่าจะออกแบบเป็น column ตายตัว ในขณะที่ความเป็นเจ้าของ สิทธิ์การเข้าถึง และโครงสร้าง workspace ยังคงอยู่ใน table เชิงสัมพันธ์ที่เคร่งครัด
>
