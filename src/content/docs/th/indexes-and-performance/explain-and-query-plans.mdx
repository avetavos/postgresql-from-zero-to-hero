...
`EXPLAIN ANALYZE` *รัน* query จริง ๆ และรายงานเวลาที่เกิดขึ้นจริงควบคู่ไปกับค่าประมาณ ตั้งแต่ PostgreSQL 18 เป็นต้นไป `ANALYZE` รายงานการใช้ buffer ให้อัตโนมัติด้วย — อ่านหน้าข้อมูล (data page) ไปกี่หน้า ซึ่งบอกคุณได้ว่างานมาจากแคชหรือจากดิสก์ — จึงไม่ต้องเขียน `BUFFERS` แยกอีกต่อไป (ในเวอร์ชันเก่ากว่า หรือถ้าต้องการเขียนให้ชัดเจน ยังใช้ `EXPLAIN (ANALYZE, BUFFERS)` ได้เหมือนเดิม และเพิ่ม `BUFFERS OFF` ถ้าต้องการปิดมันโดยเฉพาะ)
...
ก่อนที่คุณจะเพิ่ม index หรือเขียน query ใหม่ จงรัน `EXPLAIN (ANALYZE, BUFFERS)` (หรือแค่ `EXPLAIN ANALYZE` บน PostgreSQL 18 ขึ้นไป) แล้วอ่านว่า PostgreSQL ทำอะไรจริง ๆ แผนจะบอกคุณถึงชนิดของ scan, เวลาหมดไปกับอะไร และค่าประมาณของคุณตรงกับความเป็นจริงหรือไม่ — เปลี่ยนงานด้านประสิทธิภาพจากการเดาให้กลายเป็นการวัดผล
</Callout>
...
| `EXPLAIN ANALYZE` | เห็นเวลาจริง, จำนวน row จริง และ (ตั้งแต่ PostgreSQL 18) การใช้ buffer มาให้เป็นค่าเริ่มต้น | รัน query จริง ถ้าเป็น `UPDATE`/`DELETE` จะเปลี่ยนข้อมูลจริงด้วย |
| `EXPLAIN (ANALYZE, BUFFERS OFF)` | ผลลัพธ์สั้นลงอีกนิดเมื่อสนใจแค่เวลา ไม่สนใจ I/O | เสียข้อมูล cache เทียบกับ disk ที่ `Buffers` ให้ไป |
...
> 💡 ตัวอย่างจากของจริง
>
> **[pganalyze](https://pganalyze.com/)** — ดึง `EXPLAIN` plan ออกมาอัตโนมัติและติดตามแนวโน้มประสิทธิภาพ query ตามช่วงเวลา ช่วยให้ทีมจับปัญหาได้ก่อนที่ผู้ใช้จะรู้สึก
>
> **[กระบวนการ database review ของ GitLab](https://docs.gitlab.com/development/database_review/)** — กำหนดให้ merge request ที่เพิ่มหรือแก้ query ต้องแนบลิงก์ผลลัพธ์ `EXPLAIN ANALYZE` มาด้วยเสมอ เพื่อให้ reviewer เห็นผลกระทบจริงก่อน merge
