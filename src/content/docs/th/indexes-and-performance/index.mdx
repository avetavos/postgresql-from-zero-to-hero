...
- **B-tree และพื้นฐานของ index** — `CREATE INDEX`, B-tree ที่เป็นค่าเริ่มต้น, query แบบไหนที่ index ช่วยให้เร็วขึ้น, composite index และ unique index
...
export const quizIndexIntro = [
  {
    ...
    options: [
      ...
      'อ่านทุก row ใน table (Sequential Scan)',
      'เดาจาก row แรก',
      'คืนค่า error เสมอ',
    ],
    answer: 1,
  },
  {
    ...
    options: [
      'บีบอัด table เพื่อให้มีข้อมูลที่ต้องอ่านน้อยลง',
      'เป็นโครงสร้างที่เรียงลำดับไว้ ช่วยให้ server กระโดดตรงไปยัง row ที่ตรงเงื่อนไขได้ทันที',
      'แคชทั้ง table ไว้ในหน่วยความจำ',
      'ลบ row ที่ไม่ตรงเงื่อนไขออกไป',
    ],
    answer: 1,
  },
  {
    ...
    options: [
      ...
      'ไม่มีวัน — เป็นทางเลือกที่ผิดเสมอ',
