...
description: จัดอันดับ row และคำนวณยอดสะสมด้วย OVER, PARTITION BY, และ ORDER BY โดยใช้ ROW_NUMBER, RANK, DENSE_RANK, และ SUM() OVER — และดูว่าทำไม window ถึงเก็บ row ไว้ ส่วน GROUP BY ยุบทิ้ง
sidebar:
  order: 5
---
...
เมื่อคำขอบอกว่า "สำหรับแต่ละ row ให้แสดงอันดับ / ยอดสะสม / สัดส่วนของกลุ่มตัวเองด้วย" วลี "สำหรับแต่ละ row" นั้นคือสัญญาณของคุณ: คุณต้องใช้ window function ไม่ใช่ `GROUP BY` เก็บ row ไว้ เพิ่ม column เข้าไป
</Callout>
...
> 💡 ตัวอย่างจากของจริง
>
> **Activity feed และ leaderboard รายช่อง** — การจัดอันดับผู้ใช้ที่ active ที่สุดในแต่ละกลุ่มใช้ `ROW_NUMBER()`/`RANK()` แบบ `PARTITION BY` โดยตรงใน SQL แทนที่จะดึงทุกข้อความมาจัดอันดับในโค้ด
>
> **Leaderboard สรุป contribution** — รายงานสรุป contribution ต่อผู้ใช้ในแต่ละ project คำนวณอันดับด้วย window function ในฐานข้อมูล ทำให้ query เดียวได้ทั้งรายละเอียดและอันดับโดยไม่ต้อง self-join

export const quizWindows = [
  {
    q: 'window function ต่างจาก GROUP BY อย่างไร?',
    options: [
      'ยุบ row ทั้งหมดให้เหลือ row สรุปเพียง row เดียว',
      'คำนวณค่าให้ทีละ row โดยยังเก็บทุก row ไว้ในผลลัพธ์',
      'นับ row ได้อย่างเดียว',
      'ต้องมี foreign key ก่อน',
    ],
    answer: 1,
  },
  {
