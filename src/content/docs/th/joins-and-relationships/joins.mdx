...
ตัดสินใจว่า table ไหนต้องปรากฏในผลลัพธ์เสมอ ให้วาง table นั้นไว้ก่อน แล้ว `LEFT JOIN` table อีกตัวเข้ามา "ผู้เขียนทุกคน พร้อมหนังสือถ้ามี" คือ `LEFT JOIN` จาก `authors` ส่วน "เฉพาะผู้เขียนที่มีหนังสือ" คือ `INNER JOIN` วลีภาษาอังกฤษมักบอกประเภทของ join ให้คุณรู้
</Callout>
...
> 💡 ตัวอย่างจากของจริง
>
> **Activity feed และประวัติข้อความ** — แอปจำนวนมาก join table ผู้ใช้, workspace/server และข้อความตอน query แทนที่จะคัดลอกชื่อผู้ใช้ไปฝังไว้ในทุกข้อความ ทำให้เปลี่ยนชื่อครั้งเดียวก็อัปเดตทุกที่
>
> **ระบบติดตามโปรเจกต์แบบ open-source** — หน้าประวัติ merge request หรือ pipeline รวมข้อมูลจากหลาย table (users, projects, commits) ด้วย join ตอน query เพื่อให้ single source of truth อยู่ที่เดียว ไม่ต้องกังวลเรื่องข้อมูลซ้ำที่หลุดซิงค์กัน
...
export const quizJoins = [
  ...
  },
  {
    ...
    options: ['ศูนย์', 'string ว่าง', 'NULL', 'row นั้นถูกทิ้งไป'],
    answer: 2,
  },
  {
    ...
    options: [
      ...
      'ทำตัวเหมือน cross join ที่จับคู่ทุก row กับทุก row',
      'โยน syntax error ออกมาทุกครั้ง',
      'เปลี่ยนเป็น INNER JOIN บน primary key อย่างเงียบ ๆ',
    ],
    answer: 1,
  },
];
