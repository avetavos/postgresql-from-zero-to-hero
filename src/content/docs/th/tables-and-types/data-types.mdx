...
`uuid` เก็บตัวระบุที่ไม่ซ้ำกันในระดับสากลขนาด 128 บิต UUID เป็น primary key ที่ดีเมื่อ id ต้องถูกสร้างโดย client หรือต้องคาดเดาไม่ได้ เพราะมันไม่เปิดเผยจำนวน row อย่างที่ sequence ทำ
...
ชนิดใดก็ตามสามารถกลายเป็น array ได้ด้วยการเติมวงเล็บเหลี่ยม ดังนั้น `int[]` จึงเป็น column ของ array ของจำนวนเต็ม array เหมาะกับ list ขนาดเล็กที่มีลำดับซึ่งเป็นของ row เดียว เช่น tag ค่าคงที่ของ array เขียนด้วยปีกกาภายใน string ที่อยู่ในเครื่องหมายคำพูด
...
- **ใช้ชนิด `money`** — มีปัญหาเรื่อง locale และความแม่นยำที่ควบคุมยาก ทีมส่วนใหญ่จึงเลี่ยงมันใน schema ใหม่ แล้วใช้ `numeric` แทน
