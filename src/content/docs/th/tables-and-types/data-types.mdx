...
PostgreSQL 18 เพิ่ม `uuidv7()` ที่ใส่ timestamp ไว้ในบิตสูง ค่าที่สร้างออกมาจึงเรียงตามลำดับเวลาที่สร้างโดยประมาณ ลำดับนี้สำคัญมากกับ primary key เพราะ v4 ที่สุ่มล้วนจะกระจาย insert ไปทั่ว B-tree ขณะที่ v7 จะลงใกล้ขอบขวาเสมอ index จึงกระชับและเป็นมิตรกับ cache ใช้ `uuidv7()` กับตารางใหม่ และเก็บ `gen_random_uuid()` ซึ่งตอนนี้เขียนเป็น `uuidv4()` ได้ด้วย ไว้ใช้เมื่อ id ต้องไม่เปิดเผยเวลาที่สร้าง row
...
```text
                  id
--------------------------------------
 0199c4f2-8d3a-7b21-9e4c-5f6a7b8c9d0e
(1 row)
```
...
ชนิดที่ถูกต้องทำหน้าที่สองอย่างพร้อมกัน คือเก็บข้อมูลได้อย่างมีประสิทธิภาพ และปฏิเสธค่าที่ไม่ควรมีอยู่ column ที่กำหนดชนิดเป็น `numeric` สำหรับเงิน, `timestamptz` สำหรับเวลา และ `enum` สำหรับฟิลด์สถานะ เปลี่ยนบั๊กสามจำพวกให้กลายเป็นสถานะที่เป็นไปไม่ได้ การเลือกได้ดีตอนสร้างนั้นถูกกว่าการแก้ข้อมูลในภายหลัง
</Callout>
...
>
> **แพลตฟอร์มที่มีเนื้อหายืดหยุ่นสูง** — ใช้ JSONB สำหรับส่วนของ schema ที่โครงสร้างยืดหยุ่นจริง ๆ (เช่นเนื้อหา block) แต่ยังคง column แบบ typed สำหรับ metadata ที่ต้อง query และบังคับ constraint บ่อย ๆ เป็นตัวอย่างของการผสมสองแนวทางในระบบเดียว
...
export const quizDataTypes = [
  ...
  },
  {
    ...
    options: [
      'ใช้พื้นที่จัดเก็บน้อยกว่า',
      'เก็บช่วงเวลานั้นเป็น UTC แล้วแปลงตาม time zone ของ session',
      'เรียงลำดับได้เร็วกว่า',
      'ยอมให้มีค่า null ได้',
    ],
    answer: 1,
  },
  {
