...
เมื่อคุณมองหาคีย์ที่เพิ่มค่าอัตโนมัติ ให้เขียน `bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY` แบบนี้แสดงเจตนาได้ชัดเจน เป็นไปตามมาตรฐาน SQL และเลี่ยงพฤติกรรมแบบเก่าของ `SERIAL` ได้ เก็บ `SERIAL` ไว้สำหรับอ่านโค้ดเก่า ไม่ใช่สำหรับเขียน table ใหม่
</Callout>
...
| `uuid` เป็น primary key | ไม่ซ้ำกันได้ในระดับ global สร้างจาก client ได้โดยไม่ต้อง round trip ไปที่ server | ขนาดใหญ่กว่า เรียงลำดับตามเวลาไม่ได้ตามธรรมชาติ ทำให้ index กระจายตัว |
...
> 💡 ตัวอย่างจากของจริง
>
> **[Stripe](https://docs.stripe.com/api/expanding_objects)** — ใช้ id แบบ string ที่มี prefix บอกประเภท object เช่น `cus_...` สำหรับ customer หรือ `ch_...` สำหรับ charge แทนที่จะใช้ UUID ดิบ ๆ ทำให้ id ทั้ง unique และอ่านแล้วรู้ทันทีว่าเป็น object ประเภทไหน ช่วยเวลา debug log
>
> **แพลตฟอร์มที่มีเนื้อหายืดหยุ่นสูง** — มักเก็บเนื้อหาที่มีโครงสร้างยืดหยุ่นสูง (เช่นโมเดลเอกสารแบบ block) ไว้ใน column แบบ JSONB ควบคู่กับ metadata อย่าง ownership และ permission ที่ยังคงเก็บแบบ relational ปกติเพื่อให้ query และบังคับ constraint ได้ตรงไปตรงมา

export const quizCreating = [
  {
    q: 'clause ใดที่ทำให้ PostgreSQL จัดหาค่า primary-key โดยอัตโนมัติด้วย syntax มาตรฐาน SQL?',
    options: [
      'AUTO_INCREMENT',
  ...
  },
  {
    ...
    options: [
      ...
      'ลบโครงสร้าง table พร้อมทุก row อย่างถาวร',
