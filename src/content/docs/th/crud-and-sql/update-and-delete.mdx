...
description: เปลี่ยน row ด้วย UPDATE และลบด้วย DELETE — ใช้ WHERE clause เสมอ, ทำไม WHERE ที่หายไปจึงอันตราย และการใช้ RETURNING เพื่อยืนยัน
sidebar:
  order: 4
---
...
อุบัติเหตุบน production ที่พบบ่อยที่สุดอันดับหนึ่งกับ SQL คือ `UPDATE` หรือ `DELETE` ที่รันโดยไม่มี `WHERE` clause สร้างความเคยชินให้กล้ามเนื้อจดจำ: พิมพ์ `WHERE` ก่อน แล้วค่อยพิมพ์ค่าทีหลัง เมื่อสงสัย ให้ทำใน transaction ที่ rollback ได้
</Callout>
...
> 💡 ตัวอย่างจากของจริง
>
> **กระบวนการเปลี่ยนแปลงข้อมูล production ที่รัดกุม** — กำหนดให้ dry-run คำสั่ง SQL ที่ทำลายข้อมูล (destructive) ด้วยเวอร์ชัน `SELECT` ก่อนเสมอ แล้วให้ทีมรีวิวจำนวน row ก่อนอนุมัติให้รัน `UPDATE`/`DELETE` จริง
>
