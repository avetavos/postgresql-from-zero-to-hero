...
`RETURNING` เปลี่ยนการเขียน (write) ให้กลายเป็น write บวก read ในคำสั่งเดียว ช่วยตัด `SELECT` ที่ต้องตามมาทิ้ง และเลี่ยง race condition เพราะค่าที่คุณได้กลับมาคือค่าที่ insert นี้สร้างขึ้นเป๊ะ ๆ
</Callout>
...
|---------|---------|------|
| `RETURNING` หลัง `INSERT` | ได้ค่าที่ถูกสร้างขึ้น (เช่น `id`) กลับมาใน round trip เดียว ไม่มี race condition | ต้องจำ syntax เพิ่มเติม และบาง driver ต้องอ่านผลลัพธ์แบบ query แทน exec ธรรมดา |
...
| `INSERT` หลาย row ในคำสั่งเดียว | เร็ว เพราะเป็น round trip เดียว | คำสั่งยาวขึ้น และ error ของ row เดียวอาจทำให้ทั้งคำสั่งล้มเหลว |
...
> 💡 ตัวอย่างจากของจริง
>
> **งาน throughput สูง** — งาน insert ทั่วไปในแอปส่วนใหญ่ผ่าน ORM (เช่น ActiveRecord) เพื่อความสะดวกและ maintainability แต่ในจุดที่ throughput สูง เช่น event ingestion หรือ audit log ทีมมักจะลง SQL มือพร้อม `RETURNING` และ multi-row insert เพื่อลด round-trip
>
