...
เหตุที่ analytics query ที่ช้าและการ update ที่หลั่งไหลสามารถรันด้วยกันได้อย่างมีความสุขก็คือ MVCC reader เห็น snapshot ที่ถูกตรึงไว้ ส่วน writer ต่อท้ายเวอร์ชันใหม่ไว้ข้างเวอร์ชันเก่า ไม่มีใคร block ใคร ราคาที่ต้องจ่ายคือ dead tuple ที่ทิ้งไว้ข้างหลัง และนั่นคือเหตุผลที่ VACUUM มีอยู่
</Callout>
...
export const quizMvcc = [
  {
    q: 'ภายใต้ MVCC เกิดอะไรขึ้นกับ row เมื่อเราสั่ง UPDATE?',
    options: [
      'byte เดิมถูกเขียนทับที่ตำแหน่งเดิม',
  ...
  },
  {
    ...
    options: [
      'ทำให้ snapshot เสียหาย',
      'กัน VACUUM ไม่ให้ลบเวอร์ชันเก่าของ row จน table บวม',
      'ปิด autovacuum อย่างถาวร',
      'ลด isolation level ลง',
    ],
    answer: 1,
  },
];
