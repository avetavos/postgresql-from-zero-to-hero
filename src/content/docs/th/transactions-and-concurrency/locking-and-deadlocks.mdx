...
deadlock ไม่ได้เกิดจากการ lock มากเกินไป แต่เกิดจากการ lock ในลำดับที่ขัดแย้งกัน จงตัดสินใจลำดับสากลหนึ่งลำดับสำหรับการจับ lock เช่นเรียงตาม primary key จากน้อยไปมากเสมอ และนำไปใช้ในทุก transaction PostgreSQL จะยังตรวจจับ deadlock ใด ๆ ที่หลุดรอดมาได้ แต่คุณแทบจะไม่เปิดโอกาสให้เกิดขึ้นเลย
</Callout>
...
export const quizLocks = [
  {
    ...
    options: [
      ...
      'อ่านและ lock row ไว้ เพื่อให้คนอื่นต้องรอก่อนจะแก้ row นั้น',
  ...
  },
  {
    ...
    options: [
      'รอตลอดไปจนกว่า client ตัวหนึ่งจะตัดการเชื่อมต่อ',
      'ตรวจจับวงจรแล้ว abort transaction หนึ่งเป็นเหยื่อ',
      'รวมสอง transaction เข้าด้วยกัน',
      'ลด isolation level ลงโดยอัตโนมัติ',
    ],
    answer: 1,
  },
  {
